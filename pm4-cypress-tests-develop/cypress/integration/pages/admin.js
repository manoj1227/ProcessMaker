...
export class Admin {
	...
	addUserToGroup(fullName){
		cy.xpath(selectors.userTab).click();
		cy.get(selectors.newUserToGroupBtn).click();
   ...
   async getUserId(userName){
    ...
    addDashboard(name, screen, description) {
        ...
        cy.get(".form-group.mb-4").eq(0).type(name);
        cy.get(".multiselect__placeholder").type(" ");
        cy.wait(5000);
        cy.get(".multiselect__content-wrapper")
            ...
            .contains(screen)
            .click();
        cy.get(".form-group.mb-4").eq(1).type(description);
    ...
    selectMyDashboardInHomePage() {
        ...
        cy.get(".multiselect__content-wrapper").eq(0).should("be.visible");
        cy.wait(5000)
        cy.get("#listbox-0")
            .children()
            .children()
    ...
    selectDashboardToUser(nameDashboard) {
        cy.get('[class="form-group mt-3"]')
            .should("be.visible")
            .and("contain", "Dashboard");
        cy.get('[class="form-group mt-3"]')
            .eq(1)
            .type(nameDashboard, "{delay:10}")
